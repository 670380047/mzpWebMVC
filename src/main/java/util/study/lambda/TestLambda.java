...
//        printList(list);
    ...
    public static void oldIntegerSort(){
        ...
        Collections.sort(integerList, new Comparator<Integer>(){
            ...
            @Override       //lambda表达式中的入参：第一个after是数组中“靠后”的那个（即，“被比较数”），第二个before是数组中“靠前”的那个（即，“比较数”）.意思是“原有的before要和新来的after比较”
            public int compare(Integer after, Integer before) {  //源码中第一个入参（after）是数组靠后面的数(被计较的数)，第二个入参（before）是数组靠前面的数（比较的数）（比如这里：after=2，before=1）
                if(after <= before){    //由条件加上返回值来确定是升序还是降序 （如果全部返回-1（小于0）的话，则表示顺序需要调整。 返回1（大于0）则表示不需要调整位置）
